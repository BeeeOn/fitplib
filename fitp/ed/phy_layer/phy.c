...
uint8_t p_value (uint8_t band, uint8_t channel, uint8_t bitrate)
{
	uint8_t p_value = (uint8_t) (((uint16_t) (_channel_compare (band, channel, bitrate) - 75) / 76) + 1);
	return p_value;
}
...
uint8_t s_value (uint8_t band, uint8_t channel, uint8_t bitrate)
{
	...
	uint8_t s_value = (uint8_t) (((uint16_t) channel_cmp - ((uint16_t) (75 * (p_value + 1)))));
	return s_value;
}
...
void HW_irq1_occurred (void)
{
	if (PHY_STORAGE.mode == RF_RECEIVER) {
		do {
			...
			// read data until there are some
